...
def get_packages():
    try:
        ...
        packages = package_repository.list(limit, skip, sort_key, sort_order, use_typing=use_typing, **kwargs)

        result = [package.__dict__ for package in packages["result"]] if use_typing else packages["result"]
