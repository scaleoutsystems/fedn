...
class APIClient:
    ...
    def __init__(self, host: str, port: int = None, secure: Optional[bool] = None, verify: bool = False, token: str = None, auth_scheme: str = None):
        ...
        if not auth_scheme:
            auth_scheme = os.environ.get("SCALEOUT_AUTH_SCHEME", "Bearer")
        # Override potential env variable if token is passed as argument.
        if not token:
            token = os.environ.get("SCALEOUT_AUTH_TOKEN", False)
    ...
    def get_clients_count(self, only_active: bool = False):
        ...
        response = requests.get(self._get_url_api_v1("clients/count"), params=_params, verify=self.verify, headers=self.headers)
    ...
    def get_rounds(self, n_max: int = None, filter: Dict = None):
        ...
        response = requests.get(self._get_url_api_v1("rounds/"), verify=self.verify, headers=_headers, params=params)
    ...
    def run_custom_command(self, command_type: str, blocking: bool = False, timeout: int = None, client_ids: list = None, parameters: dict = None):
        ...
        response = requests.post(url, json=data, verify=self.verify, headers=self.headers)
        if response.status_code == 200:
            return response.json()
        else:
    ...
    def get_validations_count(self, session_id: str = None):
        ...
        if session_id:
            _params["session_id"] = session_id
        response = requests.get(self._get_url_api_v1("validations/count"), params=_params, verify=self.verify, headers=self.headers)
