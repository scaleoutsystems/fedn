...
class PythonEnv:
    ...
    def install_into_current_env(self, capture_output=False):
        ...
        FednLogger().info("Installing dependencies into the current environment")
    ...
    def create_virtualenv(self, capture_output=False):
        ...
        if env_dir.exists():
            FednLogger().info("Environment %s already exists", env_dir)
            return True

        with remove_on_error(
            env_dir,
            onerror=lambda e: FednLogger().warning(
            ...
            FednLogger().info("Creating a new environment in %s with %s", env_dir, sys.executable)
            _exec_cmd(
            ...
            FednLogger().info("Installing dependencies")
            for deps in filter(None, [self.build_dependencies, self.dependencies]):
                with tempfile.TemporaryDirectory() as tmpdir:
                    tmp_req_file = f"requirements.{uuid.uuid4().hex}.txt"
                    Path(tmpdir).joinpath(tmp_req_file).write_text("\n".join(deps))
