...
.. code-block:: python
    ...
    from scaleout.network.clients.edge_client import EdgeClient, ConnectToApiResult
    ...
    def main(api_url: str, api_port: int, token: str = None):
        edge_client = EdgeClient(train_callback=on_train, validate_callback=on_validate, predict_callback=on_predict)

        url = get_api_url(api_url, api_port)

        name = input("Enter Client Name: ")
        edge_client.set_name(name)

        client_id = str(uuid.uuid4())
        edge_client.set_client_id(client_id)
        ...
        result, combiner_config = edge_client.connect_to_api(url, token, controller_config)

        if result != ConnectToApiResult.Assigned:
            print("Failed to connect to API, exiting.")
            return

        result: bool = edge_client.init_grpchandler(config=combiner_config, client_name=client_id, token=token)
